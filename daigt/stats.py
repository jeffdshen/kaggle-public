...
class TotalAucRocMeter:
    ...
    @property
    def auc(self):
        score = roc_auc_score(self.labels, self.preds)
        return score
...
class SlidingAucRocMeter:
    def __init__(self, weight):
        self.max_window = int(round(1.0 / (1 - weight)))
        self.preds = []
        self.labels = []
        self.count = 0
        self.auc = 0
    ...
    def add(self, scores, count=1):
        ...
        self.labels += labels
        self.count += count
        if self.count >= self.max_window:
            self.auc = roc_auc_score(self.labels, self.preds)
            self.preds = []
            self.labels = []
            self.count = 0